...
class GraphAttentionLayer(nn.Module):
    def __init__(self,m,embedding_size, params):
        super(GraphAttentionLayer,self).__init__()
        self.l1 = nn.Parameter(torch.zeros(2*embedding_size,2*embedding_size))
        nn.init.xavier_uniform_(self.l1.data,gain=1.414)
        self.a = nn.Parameter(torch.zeros(4*embedding_size,1))
        nn.init.xavier_uniform_(self.a.data,gain=1.414)

        self.softmax = nn.Softmax(dim=-1)
        self.leaky_relu = nn.LeakyReLU(negative_slope=0.01)
        self.model = params["model_load_dir"]
    ...
    def forward(self,prev_state,candidate_action_embeddings):
        ...
        for index in range(candidate_action_embeddings.shape[2]): # number of rollouts
            ac_emb = self.leaky_relu(torch.mm(candidate_action_embeddings[:,:,index],self.l1))
            e = torch.mm(torch.cat((prev_state,ac_emb),dim=1),self.a)
            E[:,:,index] = e

        if(self.model == "saved_models/WN18RR" or self.model == "saved_models/fb15k-237"):
            pass
        else:
            candidate_action_embeddings = torch.cat((candidate_action_embeddings,candidate_action_embeddings),dim=1)

        output = torch.bmm(self.softmax(E).cuda(),candidate_action_embeddings.cuda()).sum(dim=1)
        return output


class Policy_step_entity(nn.Module):
    def __init__(self, m, embedding_size, hidden_size):
        super(Policy_step_entity, self).__init__()

        #torch.nn.BatchNorm1d(num_features, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True, device=None, dtype=None)
        self.batch_norm = nn.BatchNorm1d(m * hidden_size)
        #h_t,c_t = LSTMCell(x_t,(h_{t-1},c_{t-1}))
        self.lstm_cell = nn.LSTMCell(input_size = 2 * m * embedding_size, hidden_size= 3 * m * hidden_size) # input_size,hidden_size
        self.l1 = nn.Linear(m * embedding_size, 2 * m * embedding_size)
        self.l2 = nn.Linear(3 * m * embedding_size, m * embedding_size)
        self.l3 = nn.Linear(3 * m * embedding_size, m * embedding_size)
    ...
    def forward(self, prev_action, prev_state):
        ...
        output, ch = self.lstm_cell(prev_action, prev_state) # prev_action as input, prev_state as (ht,ct)
...
class Policy_step(nn.Module):
    def __init__(self, m, embedding_size, hidden_size):
        ...
        self.batch_norm = nn.BatchNorm1d(m * hidden_size)
        #h_t,c_t = LSTMCell(x_t,(h_{t-1},c_{t-1}))
        self.lstm_cell = nn.LSTMCell(input_size=2 * m * embedding_size, hidden_size=2 * m * hidden_size) # input_size,hidden_size
        self.l1 = nn.Linear(m * embedding_size, 2 * m * embedding_size)
...
class Agent(nn.Module):
    ...
    def step(self, next_relations, next_entities, prev_state, prev_relation, query_embedding, current_entities,
        ...
        prev_action_embedding = self.action_encoder(prev_relation, current_entities) # (original batch_size * num_rollout, 4*self.embedding_size) = (2560,200)
        ...
        candidate_action_embeddings = self.action_encoder(next_relations, next_entities)  # [2560,100,200]
        prev_entity = self.entity_embedding(current_entities)
        entity_embedding_size = prev_entity.shape[1]  # 100
        self.attention = self.GAT(prev_entity, candidate_action_embeddings)  # [2560,100]
        #self.attention = torch.cat((self.attention,self.attention),dim=1).cuda()
        # 1. one step of rnn
        ...
        prev_state = (torch.cat([prev_state[0].cuda(), entity_cluster_shared_informs[0].cuda(),self.attention.cuda()], dim=-1),

                     torch.cat([prev_state[1].cuda(), entity_cluster_shared_informs[1].cuda(), self.attention.cuda()], dim=-1)) # ([2560,600], [2560,600])
        output, new_state = self.policy_step_entity(prev_action_embedding, prev_state)
